...
for k in range(10):
    ...
    dataV, dataTest, labelsV, labelsTest = train_test_split(dataV,labelsV, test_size=0.5, random_state=40) 
    trainDS = Dataset(dataT, labelsT)
    trainDL = DataLoader(dataset = trainDS, batch_size = 20, shuffle = True)
    
    valDSS = Dataset(dataV, labelsV)
    valDLS = DataLoader(dataset = valDSS, batch_size = 20, shuffle = True)
    ...
    for epoch in range(num_epochs):
        with tqdm(trainDL, unit='batch') as tepoch:
            ...
            for images,lab in tepoch:
                ...
                tepoch.set_postfix(loss=loss.item(),totalLoss =totalLoss/len(trainDL), TrainAccuracyPerBatch= 100.*totaltrainAccuracybatch_valence)      
            model.eval()
            with torch.no_grad():
                  with tqdm(valDLS, unit='batch') as tepoch:
                        total = 0
                        correct_valence = 0
                        correct_arousal = 0
                        for images,lab in tepoch:
                            ...
                            val_loss = lossv.item()
        print('Epoch [{}], Loss_t: {:.4f}, train_acc:{:.4f}, val_acc:{:.4f}'.format(epoch, totalLoss/len(trainDL), 100.*totaltrainAccuracybatch_valence, 100.*correct_valence/total))
