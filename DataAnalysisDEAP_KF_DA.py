...
for k in range(10):
    ...
    dataTr = dataTrain[0,:,:,:]
    labelsTr = labelsTrain[0,:]
    for i in range(dataTrain.shape[0]-1):
         dataTr = np.vstack((dataTr,dataTrain[i+1,:,:,:]))
         labelsTr = np.vstack((labelsTr, labelsTrain[i+1,:]))
...
scipy.io.savemat('KF_DA.mat',{'tloss': tl,
                                     'vloss': vl,
                                     'tA': tA,
                                     'vA':vA})       